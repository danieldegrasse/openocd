			    ...
			    uint8_t *data_buf, size_t data_len)
{
	...
	if (data_len >= spi_dw_info->rx_abw) {
		/* Can't perform this transfer, RX FIFO is too small */
		LOG_ERROR("Cannot perform transfer, HW RX FIFO is too small");
		return ERROR_FAIL;
	}
			     ...
			     const uint8_t *data_buf, size_t data_len)
{
	...
	if (spi_dw_info->tx_abw < (1 + addr_len + data_len)) {
		/* Can't perform this transfer, TX FIFO is too small */
		LOG_ERROR("Cannot perform transfer, HW TX FIFO is too small");
		return ERROR_FAIL;
	}
			...
			uint32_t offset, uint32_t count)
{
	...
	while (i < count) {
		write_len = MIN(count - i, (spi_dw_info->tx_abw - (1 + addr_len)) - 1);
		if ((offset + i) % fdev->pagesize + write_len > fdev->pagesize) {
			/* Write would cross page boundary */
			write_len = fdev->pagesize - ((offset + i) % fdev->pagesize);
		}
		/* Set write enable */
		rc = spi_dw_write_addr(bank, SPIFLASH_WRITE_ENABLE, NULL, 0x0,
				       NULL, 0x0);
